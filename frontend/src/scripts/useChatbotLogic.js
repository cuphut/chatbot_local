...
export const useChatbotLogic = () => {
  ...
  const [messages, setMessages] = useState(() => {
    ...
    return savedMessages ? JSON.parse(savedMessages) : [
      { from: "bot", text: "Hello, how can I help you? 😊", timestamp: new Date() },
    ];
  });
  ...
  const availableModels = [
    { name: "Gemini 2.0", value: "google/gemini-2.0-flash-exp:free" },
    { name: "Gemma 3", value: "google/gemma-3-27b-it:free" },
    { name: "LLaMA 4", value: "meta-llama/llama-4-scout:free" },
  ];
  ...
  const clearChatHistory = useCallback(() => {
    ...
    if (hasUserMessages) {
      ...
      setMessages([{ from: "bot", text: "Hello, how can I help you? 😊", timestamp: new Date() }]);
      
      // Hiển thị thông báo có thể tắt
      setClearNotice({
        id: Date.now(),
        text: "Chat history has been automatically cleared due to inactivity."
      });
    }
  },[setMessages]);
